...
use llvm_sys::{
    ...
    core::{
        ...
        LLVMFunctionType, LLVMInt32Type, LLVMModuleCreateWithName, LLVMPositionBuilderAtEnd,
        LLVMVoidType,
    },
    LLVMModule,
};
    ...
    while let Some(node) = nodes.peek() {
        match node {
            Stmt::Function { name, nodes } => {
                ...
                let func_ty = LLVMFunctionType(LLVMInt32Type(), [].as_mut_ptr(), 0, 0);
                ...
                let strptr = LLVMBuildGlobalStringPtr(
                    builder,
                    cstr!("Snout is cute"),
                    cstr!("only_global_var"),
                );
